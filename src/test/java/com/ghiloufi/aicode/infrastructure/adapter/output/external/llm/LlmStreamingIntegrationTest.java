 ...
 *
 * <p>This test demonstrates true LLM integration testing with: - Real LLM inference via Ollama
 * TestContainer - Streaming response validation - Timeout and error handling - Performance
 * characteristics
 *
...
class LlmStreamingIntegrationTest {
  ...
  @Autowired private OllamaLlmStreamingAdapter ollamaAdapter;
  ...
  void testStreamingLlmAnalysis() {
    ...
    log.info(
        "Streaming test completed: {} chunks, {} total characters",
        chunkCount.get(),
  ...
  void testStreamingTimeout() {
    ...
    StepVerifier.create(
            ollamaAdapter
                .streamAnalyzeDiff(largeDiffAnalysis, config)
  ...
  void testStreamingErrorHandling() {
    ...
    DiffAnalysis diffAnalysis = createTestDiffAnalysis();
    ReviewConfiguration invalidConfig =
