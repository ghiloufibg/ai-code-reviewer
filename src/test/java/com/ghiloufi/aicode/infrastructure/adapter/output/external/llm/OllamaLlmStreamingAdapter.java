 ...
 *
 * <p>This adapter provides real LLM integration for testing streaming responses and actual AI
 * analysis behavior in integration tests.
 */
@TestComponent
...
public class OllamaLlmStreamingAdapter implements LlmAnalysisPort {
  ...
  private AnalysisResult combineStreamingChunks(List<String> chunks, DiffAnalysis diffAnalysis) {
    ...
    log.info(
        "Combined {} streaming chunks into response of {} characters",
        chunks.size(),
        fullResponse.length());
  ...
  private String extractContentFromChunk(String chunk) {
    try {
      ...
      if (chunk.contains("\"response\":")) {
        ...
        if (end > start) {
          return chunk.substring(start, end).replace("\\\\n", "\\n").replace("\\\\\"", "\"");
        }
      }
      return "";
    } catch (Exception e) {
