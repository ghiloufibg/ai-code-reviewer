 ...
 *
 * <p>This test demonstrates how to test streaming behavior without
 * running actual LLM infrastructure, making it perfect for:
 * - Unit tests
...
class MockLlmStreamingTest {
    ...
    void testMockStreamingResponse() {
        ...
        StepVerifier.create(streamingResponse)
            ...
            .thenConsumeWhile(chunk -> {
                ...
                log.info("Received mock chunk #{}: {}", count,
        ...
        log.info("Mock streaming test completed: {} chunks, {} total characters",
    ...
    void testFullAnalysisWithMockStreaming() {
        ...
        StepVerifier.create(mockAdapter.analyzeDiff(testDiffAnalysis, testConfig))
            .assertNext(analysisResult -> {
                ...
                assertThat(analysisResult.getNotes())
                    .contains("Mock streaming analysis")
                    .hasSize(3);

                log.info("Mock analysis completed: {}", analysisResult.getSummary().substring(0, 100));
            })
            .expectComplete()
            .verify(Duration.ofSeconds(5));
    }
    ...
    void testStreamingTimeout() {
        ...
        Flux<String> slowResponse = mockAdapter.simulateStreamingWithTimeout()
    ...
    private ReviewConfiguration createTestReviewConfiguration() {
        return new ReviewConfiguration(
            "mock-model", null, 30, 1000, 3, "17", "maven",
            false, true, false
        );
    }
}