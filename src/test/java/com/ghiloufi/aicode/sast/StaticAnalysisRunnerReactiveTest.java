 ...
 *
 * <p>Ces tests vérifient tous les aspects réactifs du StaticAnalysisRunner, y compris
 * la lecture de fichiers concurrente, la gestion d'erreurs, et la compatibilité backwards.
 */
...
class StaticAnalysisRunnerReactiveTest {
  ...
  class ConstructorTests {
    ...
    void testConstructorWithInvalidMaxContentLength() {
      assertThrows(IllegalArgumentException.class,
          () -> new StaticAnalysisRunner(tempDir, 0));
      assertThrows(IllegalArgumentException.class,
          () -> new StaticAnalysisRunner(tempDir, -1));
    }
  }
  ...
  class ReactiveCollectionTests {
    ...
    void testRunAndCollectReactiveWithLargeContent() throws IOException {
      ...
      for (int i = 0; i < 10000; i++) {
        largeContent.append("<error line=\"").append(i).append("\" message=\"Long error message for line ").append(i).append("\"/>\n");
      }
  ...
  class UtilityMethodTests {
    ...
    void testIsTargetDirectoryAccessible() {
      ...
      StaticAnalysisRunner nonExistentRunner = new StaticAnalysisRunner(tempDir.resolve("non-existent"));
  ...
  class ConcurrentProcessingTests {
    ...
    void testConcurrentFileProcessing() throws IOException {
      ...
      Files.writeString(tempDir.resolve("checkstyle-result.xml"),
          "<checkstyle>checkstyle content</checkstyle>");
      Files.writeString(tempDir.resolve("pmd.xml"),
          "<pmd>pmd content</pmd>");
      Files.writeString(tempDir.resolve("spotbugs.xml"),
          "<spotbugs>spotbugs content</spotbugs>");
      Files.writeString(tempDir.resolve("semgrep.json"),
          "{\"tool\": \"semgrep\", \"results\": []}");
      ...
      StepVerifier.create(staticAnalysisRunner.runAndCollect())
          .expectNextMatches(results -> {
            ...
            // (This is a basic check - in real scenarios concurrent processing shows more benefit)
