 ...
 *
 * <p>This test ensures that: 1. Clean Architecture ports work correctly with streaming 2. Use case
 * orchestration handles streaming responses properly 3. Domain entities are created and updated
 * correctly 4. The full workflow from command to result works end-to-end
 */
@ExtendWith(MockitoExtension.class)
@Slf4j
class StartReviewUseCaseStreamingTest {

  @Mock private DiffCollectionPort diffCollectionPort;

  @Mock private ReviewRepository reviewRepository;

  @Mock private StaticAnalysisPort staticAnalysisPort;

  @Mock private ReviewPublishingPort reviewPublishingPort;

  @Mock private ReviewPolicyService reviewPolicyService;
  ...
  void shouldExecuteFullReviewWorkflowWithStreamingLlm() {
    ...
    when(staticAnalysisPort.isAvailable())
        .thenReturn(false); // Disable static analysis for this test
    when(reviewPublishingPort.supports(any()))
        .thenReturn(false); // Disable publishing for this test
            ...
            review -> {
              ...
              AnalysisResult llmResult =
                  review.getAnalysisResults().stream()
                      .filter(
                          result1 -> result1.getType() == AnalysisResult.AnalysisType.LLM_ANALYSIS)
                      .findFirst()
                      .orElseThrow(
                          () -> new AssertionError("LLM analysis result should be present"));
              ...
              assertThat(llmResult.getNotes()).contains("Mock streaming analysis").hasSize(3);

              log.info(
                  "📝 LLM Analysis Summary: {}",
                  llmResult
                      .getSummary()
                      .substring(0, Math.min(100, llmResult.getSummary().length())));
    ...
    StepVerifier.create(startReviewUseCase.execute(command))
        .assertNext(
            review -> {
              ...
              boolean hasLlmAnalysis =
                  review.getAnalysisResults().stream()
                      .anyMatch(
                          result ->
                              result.getType() == AnalysisResult.AnalysisType.LLM_ANALYSIS
  ...
  private StartReviewCommand createTestCommand() {
    RepositoryInfo repositoryInfo =
        RepositoryInfo.forLocalCommits("test-repo", "HEAD~1", "HEAD", "main");
