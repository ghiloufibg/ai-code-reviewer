 ...
 *
 * <p>This adapter maintains the same interface as the old CodeReviewOrchestrator
...
public class LegacyCodeReviewOrchestrator {
    ...
    /**
     * Executes code review with legacy ApplicationConfig.
     * Maintains compatibility with existing controllers.
     */
    public Mono<Void> executeCodeReview(ApplicationConfig config) {
        ...
        return reviewManagementPort.startReview(command)
    ...
    private RepositoryInfo createRepositoryInfo(ApplicationConfig config) {
        if ("github".equals(config.getMode())) {
            return RepositoryInfo.forGitHubPR(
