 ...
 *
 * <p>This adapter coordinates between GitHub API and local Git to collect diff data based on the
...
public class DiffCollectionAdapter implements DiffCollectionPort {
  ...
  public Mono<DiffAnalysis> collectDiff(RepositoryInfo repositoryInfo) {
    ...
    if (repositoryInfo.isGitHubMode()) {
      return gitHubAdapter
          .getDiff(repositoryInfo.repository(), repositoryInfo.pullRequestNumber())
          .map(this::parseDiff);
    } else {
      return localGitAdapter
          .getDiff(repositoryInfo.fromCommit(), repositoryInfo.toCommit())
          .map(this::parseDiff);
    }
  }
  ...
  /**
   * Parses raw diff into structured DiffAnalysis. This is a simplified implementation - in reality
   * you'd use the existing UnifiedDiffParser.
   */
