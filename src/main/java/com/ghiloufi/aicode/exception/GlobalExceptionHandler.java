 ...
 *
 * <p>Cette classe centralise la gestion des exceptions pour l'application,
 * offrant une approche cohérente pour la gestion des erreurs et la création
 * de réponses d'erreur standardisées.
 *
 * <p>Utilise les modèles générés par OpenAPI pour assurer la cohérence
 * avec la spécification de l'API.
 */
@ControllerAdvice
@Slf4j
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleReviewInProgressException(ReviewInProgressException ex) {
    ...
    public ResponseEntity<ErrorResponse> handleValidationException(Exception ex) {
        ...
        if (ex instanceof WebExchangeBindException bindEx) {
            message = bindEx.getBindingResult().getAllErrors().stream()
    ...
    public ResponseEntity<ErrorResponse> handleServerWebInputException(ServerWebInputException ex) {
        ...
        ErrorResponse error = createErrorResponse(
            "INVALID_INPUT",
            "Invalid request format: " + ex.getReason(),
            HttpStatus.BAD_REQUEST
        );
