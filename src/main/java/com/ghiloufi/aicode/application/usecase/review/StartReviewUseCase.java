 ...
 *
 * <p>Orchestrates the entire review process from diff collection
 * through analysis and result publication.
 */
@UseCase
...
public class StartReviewUseCase {
    ...
    private Mono<CodeReview> collectDiffAndAnalyze(CodeReview review) {
        return diffCollectionPort.collectDiff(review.getRepositoryInfo())
            .doOnNext(diffAnalysis -> {
                review.addDiffAnalysis(diffAnalysis);
                log.info("Collected diff: {} lines across {} files",
     ...
     */
    private Mono<CodeReview> performAnalysis(CodeReview review, com.ghiloufi.aicode.domain.entity.DiffAnalysis diffAnalysis) {
        ...
        if (review.getConfiguration().enableStaticAnalysis() && staticAnalysisPort.isAvailable()) {
            staticAnalysis = staticAnalysisPort.analyze(diffAnalysis)
                .doOnNext(result -> {
        ...
        if (review.getConfiguration().enableLlmAnalysis() && aiAnalysisPort.isAvailable()) {
            aiAnalysis = aiAnalysisPort.analyze(diffAnalysis, review.getConfiguration())
                .doOnNext(result -> {
                    review.addAnalysisResult(result);
    ...
    private ReviewResult finalizeReview(CodeReview review) {
        ...
        var allIssues = review.getAnalysisResults().stream()
            .flatMap(result -> result.getIssues().stream())
            .toList();

        var allNotes = review.getAnalysisResults().stream()
            .flatMap(result -> result.getNotes().stream())
            .toList();
        ...
        log.info("Review completed: {} issues found, quality: {}",
                finalResult.getIssueCount(), finalResult.getQuality());
    ...
    private String generateSummary(CodeReview review) {
        int totalIssues = review.getAnalysisResults().stream()
            .mapToInt(AnalysisResult::getIssueCount)
            .sum();

        return String.format("Code review completed for %s. Found %d issue(s) across %d analysis tool(s).",
    ...
    private Mono<CodeReview> publishReviewIfNeeded(CodeReview review, ReviewResult result) {
        if (reviewPublishingPort.supports(review)) {
            return reviewPublishingPort.publishReview(review, result)
