 ...
 *
 * <p>This service coordinates use cases and provides the implementation
 * of the ReviewManagementPort interface.
 */
@Service
...
public class ReviewManagementService implements ReviewManagementPort {
    ...
    @Override
    public Flux<CodeReview> listReviews(String repository, CodeReview.ReviewStatus status, int page, int size) {
        // Simple implementation - in production you'd want proper pagination
        if (status != null) {
            return reviewRepository.findByStatus(status)
                .filter(review -> repository == null || repository.equals(review.getRepositoryInfo().repository()))
                .skip((long) page * size)
                .take(size);
        } else if (repository != null) {
            return reviewRepository.findByRepository(repository)
                .skip((long) page * size)
                .take(size);
        } else {
